...
class BlocklistService {
  ...
  Future<bool> updateBlocklist() async {
    try {
      ...
      final blockResponse = await http
          .get(Uri.parse(_defaultSourceUrl))
      ...
      for (var url in _globalWhitelists) {
        try {
          debugPrint(
            'ZeroAd_Intelligence: Mengunduh Whitelist dari ${url.split('/').last}...',
          );
          ...
          debugPrint(
            'ZeroAd_Intelligence: Gagal mengambil whitelist dari $url: $e',
          );
        }
      }
      ...
      if (blockResponse.statusCode == 200) {
        debugPrint(
          'ZeroAd_Intelligence: Memproses dan menyaring data (Scrubbing)...',
        );

        // Proses Blokir di Isolate
        final cleanedBlock = await compute(
          _scrubAndSanitize,
          blockResponse.body,
        );
        ...
        if (aggregatedWhitelists.isNotEmpty) {
          debugPrint(
            'ZeroAd_Intelligence: Menyimpan ${aggregatedWhitelists.length} domain intelijen...',
          );
  ...
  Future<File> _getWhitelistFile() async => File(
    '${(await getApplicationDocumentsDirectory()).path}/$_whitelistFileName',
  );
